...
/// Upstream's repository root, relative to this one.
///
/// A handful of these tests assert on repo files rather than on `just`'s
/// behaviour — the changelog `--changelog` prints, the example justfiles, the
/// README's heading conventions. Upstream had them at its crate root; here they
/// live alongside the vendored source. This is the only edit made to the
/// vendored suite. See `tests/integration/README.md`.
const V1_ROOT: &str = "crates/just-v1";
